...
const Walletwithdraw = ({ dispatch, navigation, providerData, amountWithdrawdata }) => {
    ...
    console.log(providerData?._id, 'data')
    ...
    function withdrawamount() {
        ...
        return (
            ...
            <View style={{ height: SCREEN_HEIGHT * 0.8, backgroundColor: "white", borderTopLeftRadius: 30, borderTopRightRadius: 30, paddingHorizontal: SCREEN_WIDTH * 0.05, paddingVertical: SCREEN_WIDTH * 0.1 }}>
                <View style={{ borderWidth: 1, borderColor: '#FE6006', borderRadius: 50, paddingHorizontal: SCREEN_WIDTH * 0.05 }}>
                    <TextInput 
                ...
                <TouchableOpacity style={{ backgroundColor: colors.new_color, paddingVertical: SCREEN_HEIGHT * 0.02, alignItems: "center", borderRadius: 50, marginTop: Sizes.fixPadding * 2 }}
                  onPress={() => {
                    const walletBalance = parseFloat(showNumber(providerData?.wallet_balance).replace(/[^0-9.]/g, "")); // Remove ₹ and parse as float
                    const withdrawalAmount = parseFloat(amount); // Parse entered amount
                
                    console.log("Withdrawal Amount:", withdrawalAmount, "Wallet Balance:", walletBalance);
                
                    if (isNaN(withdrawalAmount)) {
                        showToastMessage({ message: "Please enter a valid numeric amount." });
                    } else if (withdrawalAmount > walletBalance) {
                        showToastMessage({ message: "You cannot withdraw more than your account balance." });
                    } else if (withdrawalAmount <= 0) {
                        showToastMessage({ message: "Please enter a valid amount greater than 0." });
                    } else {
                        handlepress();
                    }
                }}
