    ...
    return (
      ...
      <View style={{padding: Sizes.fixPadding * 1.5,}}>
        <Text
          textBreakStrategy='highQuality'
          style={{ ...Fonts.black14RobotoRegular, }}
        >{astroData?.long_bio.length > 350 && !seeMore1 ? astroData?.long_bio.slice(0, 350) : astroData?.long_bio}<Text onPress={() => updateState({ seeMore1: !seeMore1 })} style={{ color: '#FE6006' }}> {astroData?.long_bio.length > 350 ? seeMore1 ? '...Read less' : '...Read more' : ''}</Text></Text>
      </View>
      </View>
