...
function* onAcceptRejectChat(actions) {
    try {
        ...
        console.log("Request Daata ::: ", requestData);
...
function* onInitiatChat(actions) {
    try {
        ...
        const response = yield postRequest({
            url: 'https://api.astrofriends.in/api/customers/get_chat_details',
            data: {
                chatId: historyId,
            }
        })
        ...
        const astrologerResponse = yield postRequest({
            url: 'https://api.astrofriends.in/api/astrologer/get_splash',
            data: {
...
function* onVideoCallENd(actions) {
    try {
        const { payload } = actions
        console.log("call Id Payload video :::", payload);
        // const data ={
        //     callId: payload.callID,
        // }
        // const response = axios.post(api_url + endvidocall,data);
        const response = yield postRequest({
            url: `https://api.astrofriends.in/api/customers/endvideocall`,
            data: {
                callId: payload.callID,
            }
        })
