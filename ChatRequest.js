...
const ChatRequest = () => {
  ...
  const onAccept = async () => {
    try {
      ...
      await axios.post('https://api.astrofriends.in/api/customers/accept_chat', { chatId: notificationData?.notification?.data?.historyId, type: 'astrologer' })
      BackHandler.exitApp()
      Linking.openURL(`astrofriends-64b40://chat/:${notificationData?.notification?.data?.chatId}/:${notificationData?.notification?.data?.historyId}`)
    } catch (e) {
      console.log(e)
    }
  }
  ...
  const onReject = async () => {
    try {
      await notifee.cancelNotification(notificationData?.notification?.id);
      await axios.post('https://api.astrofriends.in/api/customers/reject_chat', { chatId: notificationData?.notification?.data?.historyId, type: 'astrologer' })
      BackHandler.exitApp()
    } catch (e) {
      console.log(e)
    }
