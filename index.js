...
Notifee.onForegroundEvent(async ({ type, detail }) => {
  ...
  if (type === EventType.ACTION_PRESS && detail.pressAction.id === 'chat_accept') {
    ...
    Linking.openURL(`astrofriends-64b40://chat/:${detail.notification.data.chatId}/:${detail.notification.data.historyId}`)
    await axios.post('https://api.astrofriends.in/api/customers/accept_chat', { chatId: detail.notification.data.historyId, type: 'astrologer' })


  } else if (type === EventType.ACTION_PRESS && detail.pressAction.id === 'chat_reject') {
    console.log('User pressed an action with the id: ', detail.pressAction.id);
    await Notifee.cancelNotification(detail.notification.id);
    await axios.post('https://api.astrofriends.in/api/customers/reject_chat', { chatId: detail.notification.data.historyId, type: 'astrologer' })
  }
});
